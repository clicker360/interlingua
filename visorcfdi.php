<?php
define('RutaFTP', "/home/scmcfdi/", false);
...
function ObtenerCFDi() {
	...
	if (is_dir($emisor) && is_dir($receptor)) {
		...
		if (count($cfdi) > 0) {
			ksort($cfdi);
			$link = '<a href="#" onclick="mostrar(\'%s\');" >%s</a>';
			...
			foreach($cfdi as $k => $v) {
				$html .= sprintf($fila, 
					sprintf($link, urlencode($v['xml']), "XML"),
					$v['pdf'] == '' ? "": sprintf($link, urlencode($v['pdf']), "PDF"),
...
<head>
	...
	function mostrar(archivo) {
		var a = archivo.toLowerCase();
		if (a.indexOf(".xml") > 0 || a.indexOf(".pdf") > 0) {
			window.open("visorarchivo.php?a=" + archivo,'','width=700,height=700');
		}
	}
	</script>
</head>
<body>
	...
	<form method="post">
		<div class="contenido">
			<label for="Emisor">RFC Emisor:</label> <input type="text" name="Emisor" value="<?php echo @$_POST['Emisor']; ?>" maxlength="15" size="15" />
			<span style="display:inline-table; width:70px"></span>
			<label for="Receptor">RFC Receptor:</label> <input type="text" name="Receptor" value="<?php echo @$_POST['Receptor']; ?>" maxlength="15" size="15" />
			<br />
